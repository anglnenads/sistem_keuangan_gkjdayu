...
if (!empty($bulan)) {
    if ($bulan == 0) {
        ...
        $sql .= " WHERE f.tahun = $tahun AND p.status = 'Tervalidasi' AND month(tanggal_penerimaan) = $bulan ";
        $laporan = 'BULAN ' . strtoupper($nama_bulan) . ' TAHUN ' . $tahun;
        $title = 'Laporan Penerimaan Gereja ' . $nama_bulan . ' ' . $tahun;
    }
} else {
    $sql .= " WHERE f.tahun = $tahun AND p.status = 'Tervalidasi'";
    $laporan = 'TAHUN ' . $tahun;
    $title = 'Laporan Penerimaan Gereja ' . $tahun;
}
...
foreach ($groupedData as $tanggal => $tanggalData) {
    ...
    foreach ($tanggalData as $jenis => $dataList) {
        $data1 = $data1 . '<tr style="font-weight:bold">
        ...
        foreach ($dataList as $data) {
            ...
            $data1 = $data1 . "<tr>
                                <td></td>
                                <td style='padding-left: 10px;'>" . $data["nama_akun"] . '</td>
                                <td style="text-align: right;">' . number_format($data["jumlah_penerimaan"], 0, ',', '.') . "</td>
                                <td></td>
                            </tr>";
        }
...
$tabelfooter = '<tr style="font-weight:bold">
                    <td width="33%">Total Penerimaan</td>
                    <td></td>
...
$pdf->Cell(0, 10, 'LAPORAN PENERIMAAN', 0, 1, 'C');
