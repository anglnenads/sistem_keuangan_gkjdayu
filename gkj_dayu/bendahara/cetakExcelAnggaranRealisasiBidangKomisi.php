...
$periodeLabel = $start == $end ? "_{$nama_bulan[$start]}" : "_{$nama_bulan[$start]}-{$nama_bulan[$end]}";
$namaFile = "laporanRencanaRealisasiPengeluaran_{$tahun}{$periodeLabel}.xls";
...
// Query Anggaran, filter berdasarkan tahun & bidang (jika dipilih bidang != 0)
$q_anggaran = "SELECT k.nama_komisi, p.nama_program, ak.nama_akun,
...
if ($bidang && $bidang != 0) {
    $q_anggaran .= " AND b.id_bidang = {$bidang}";
}

$q_anggaran .= " ORDER BY k.nama_komisi, p.nama_program";

$res_anggaran = $db->query($q_anggaran);
$data_anggaran = [];
while ($r = $res_anggaran->fetch_assoc()) {
    $data_anggaran[$r['nama_komisi']][$r['nama_program']][] = $r;
}

// Query Realisasi Pengeluaran, filter tahun, bulan antara start s.d end, dan bidang (jika ada)
$q_pengeluaran = "
    SELECT k.nama_komisi, p.nama_program, ak.nama_akun,
    ...
    LEFT JOIN bidang b ON p.id_bidang = b.id_bidang
    WHERE f.tahun = ?
      AND MONTH(pg.tanggal_pengeluaran) BETWEEN ? AND ?
      AND pg.status = 'Tervalidasi'";

$params = [$tahun, $start, $end];
$types = "iii";

if ($bidang && $bidang != 0) {
    $q_pengeluaran .= " AND b.id_bidang = ?";
    $types .= "i";
    $params[] = $bidang;
}

$q_pengeluaran .= " ORDER BY k.nama_komisi, p.nama_program, pg.tanggal_pengeluaran";
...
while ($r = $res_pengeluaran->fetch_assoc()) {
    $data_pengeluaran[$r['nama_komisi']][$r['nama_program']][] = $r;
}

// CETAK TABEL ANGGARAN
echo "<table border='1' cellpadding='5' cellspacing='0' style='border-collapse: collapse; width:100%;'>";
echo "<tr><th colspan='7'>TOTAL ANGGARAN - Periode {$tahun} {$periodeLabel}</th></tr>";
echo "<tr>
    <th>Akun</th>
    <th>Item</th>
    <th>Harga Satuan</th>
...
foreach ($data_anggaran as $komisi => $programs) {
    echo "<tr><td colspan='7' style='font-weight:bold; text-align:left;'>&nbsp;</td></tr>";
    echo "<tr><td colspan='7' style='font-weight:bold; text-align:left;'>Komisi: " . htmlspecialchars($komisi) . "</td></tr>";

    $totalKomisiG = 0; $totalKomisiS = 0; $totalKomisiT = 0;
    ...
    foreach ($programs as $program => $rows) {
        ...
        $totalKomisiG += $totalProgramG;
        $totalKomisiS += $totalProgramS;
        $totalKomisiT += $totalProgramT;
    }

    echo "<tr style='font-weight:bold;'>
        <td colspan='4'>Total Anggaran Komisi " . htmlspecialchars($komisi) . "</td>
        <td>" . formatRupiah($totalKomisiG) . "</td>
        <td>" . formatRupiah($totalKomisiS) . "</td>
        <td>" . formatRupiah($totalKomisiT) . "</td>
    </tr>";
}
echo "</table><br><br>";
...
echo "<table border='1' cellpadding='5' cellspacing='0' style='border-collapse: collapse; width:100%;'>";
echo "<tr><th colspan='8'>TOTAL PENGELUARAN - Periode {$tahun} {$periodeLabel}</th></tr>";
echo "<tr>
    <th>Tanggal</th>
    <th>Akun</th>
    <th>Item</th>
...
foreach ($data_pengeluaran as $komisi => $programs) {
    echo "<tr><td colspan='8'>&nbsp;</td></tr>";
    echo "<tr><td colspan='8' style='font-weight:bold; text-align:left;'>Komisi: " . htmlspecialchars($komisi) . "</td></tr>";
    ...
    foreach ($programs as $program => $rows) {
        ...
        foreach ($rows as $r) {
            echo "<tr>
                <td>" . htmlspecialchars(date('d-m-Y', strtotime($r['tanggal_pengeluaran']))) . "</td>
                <td>" . htmlspecialchars($r['nama_akun']) . "</td>
                <td>" . htmlspecialchars($r['item']) . "</td>
                <td>" . formatRupiah($r['harga_satuan']) . "</td>
                <td>" . htmlspecialchars($r['volume']) . "</td>
    ...
    echo "<tr style='font-weight:bold;'>
        <td colspan='5'>Total Pengeluaran Komisi " . htmlspecialchars($komisi) . "</td>
        <td>" . formatRupiah($totalKomisiG) . "</td>
        <td>" . formatRupiah($totalKomisiS) . "</td>
        <td>" . formatRupiah($totalKomisiT) . "</td>
    </tr>";
