...
$namaFile = "laporanRencanaRealisasiPerKomisi_{$tahun}" . ($bulan ? "_{$nama_bulan[$bulan]}" : "_Januari-Desember") . ".xls";
...
// Ambil data ANGGARAN, grup by komisi & program
$q_anggaran = "SELECT k.nama_komisi, p.nama_program, ak.nama_akun,
        ...
        WHERE f.tahun = {$tahun}
        ORDER BY k.nama_komisi, p.nama_program";
...
// Ambil data REALISASI PENGELUARAN, grup by komisi & program, status = 'Tervalidasi'
$q_pengeluaran = "
...
foreach ($data_anggaran as $komisi => $programs) {
    echo "<tr><td colspan='7' style='font-weight:bold; text-align:left;'>&nbsp;</td></tr>"; // Spasi antar komisi
...
foreach ($data_pengeluaran as $komisi => $programs) {
    echo "<tr><td colspan='8'>&nbsp;</td></tr>"; // Spasi antar komisi
