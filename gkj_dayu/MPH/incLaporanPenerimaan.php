...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <?php
            _myHeader("document", "Laporan Penerimaan ", "Laporan ");
            ?>
        </div>
    </div>
    ...
    <div class="second" style="display: flex; justify-content: space-between; align-items: center; width: 100%; color:#003153; font-weight:500">
        ...
        <div style="display: flex; justify-content: end; width:30%;">
            <div class="" style="text-align:center; background-color:#2e8b57; width: 23%; color:white; border-radius:4px">
                <a href="http://localhost:80/gkj_dayu/Cetak/xlxCetakPenerimaan.php" target="" style="color:white; text-decoration: none;">Cetak Excel</a>
            </div> &nbsp;&nbsp;
            <div class="" style="text-align:center; background-color:#dc143c; width: 23%; color:white; border-radius:4px">
                <a href="http://localhost:80/gkj_dayu/Cetak/pdfCetakPenerimaan.php" target="_new" style="color:white; text-decoration: none;">Cetak PDF</a>
            </div>
        </div>
    </div>
    ...
    <div class="sub-title" style=" justify-content: space-between; align-items: center; ">
        <p>LAPORAN PENERIMAAN GEREJA
            ...
            } elseif (isset($_POST['filter']) && $_POST['filter'] == 'bulan') {
                if ($_POST["tb_bulan"] == 0) {
                    echo "<span>TAHUN $tahun_aktif</span>";
                } else {
            ...
            } elseif (isset($_POST['filter']) && $_POST['filter'] == 'tahun') {
                if ($_POST["tb_tahun"] == 0) {
                    echo "<span>TAHUN $tahun_aktif</span>";
                } else {
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            if (isset($_POST['filter']) && $_POST['filter'] == 'bulan') {
                if ($_POST["tb_bulan"] == 0) {
                    $sql .= "WHERE f.tahun = $tahun_aktif AND p.status = 'Tervalidasi'";
                } else {
                    $nomorBulan = intval($bulan);
                    $sql .= " WHERE f.tahun = $tahun_aktif AND month(tanggal_penerimaan) = $nomorBulan AND p.status = 'Tervalidasi'";
                }
            } else if (isset($_POST['filter']) && $_POST['filter'] == 'tahun') {
                if ($_POST["tb_tahun"] == 0) {
                    $sql .= "WHERE f.tahun = $tahun_aktif  AND p.status = 'Tervalidasi'";
                } else {
                    $sql .= "WHERE f.tahun = " . $_POST["tb_tahun"] . " AND p.status = 'Tervalidasi'";
                }
            } else {
                $sql .= "WHERE f.tahun = $tahun_aktif  AND p.status = 'Tervalidasi'";
            }

            $sql .= " ORDER BY tanggal_penerimaan ASC";
                    ...
                    <tbody>
                        ...
                        foreach ($groupedData as $tanggal => $tanggalData) {
                            foreach ($tanggalData as $jenis => $dataList) {
                                echo "<tr>
                                        <td colspan='2' style='white-space: pre;'>" . date('d-m-Y', strtotime($tanggal)) . "           $jenis</td>
                                        <td width=''></td>
                                        <td></td>
                                        <td></td>
                                        <td></td>
                                ...
                                foreach ($dataList as $data) {
                                    $total_penerimaan = $total_penerimaan + $data["jumlah_penerimaan"];
                        ?>
                                    <tr>
                                        <td></td>
                                        <td><?= $data["nama_akun"]; ?></td>
                                        <td class="text-end">
                                            <?= ($data["jumlah_penerimaan"] == 0) ? '-' : number_format($data["jumlah_penerimaan"], 0, ',', '.'); ?>
                                        </td>
                                        <td></td>
                                        <td></td>
                                        <td></td>
                                ...
                                <tr>
                                    <td></td>
                                    <td style="font-weight:bolder">Total</td> <!-- Total per jenis penerimaan -->
                                    <td></td>
                                    <td class="text-start" style="font-weight:bolder"><?= number_format($total_penerimaan, 0, ',', '.') ?></td>
                                    <td></td>
                                    <td></td>
                                </tr>
                        <?php
                                $all_total = $all_total + $total_penerimaan;
                            }
                        }
                        ?>
                        <tr>
                            <td></td>
                            <td style="color:#5B90CD; font-weight:bolder">Total Penerimaan</td> <!-- Total keseluruhan -->
                            <td></td>
